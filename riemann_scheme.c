...
void* calcular(void *arg) {
	...
	int id = (int)arg;//primer paso
	
	double loslimit = 0;//segundo paso
	
	partialSums[id] = 0;//tercer paso
	
	int i;//cuarto paso
	base_length = (double)(length/MAXRECT);
	for( i = id*numRectxThread+loslimit; i < (id+1)*numRectxThread+loslimit; i++){
		partialSums[id] += function(i)*base_length;//quinto paso
		}
...
int main(int argc, char** argv) {
	...
	long taskids[MAXTHREAD]; //almacena los identificadores de los hilos
	...
	for (int i = 0; i < MAXTHREAD; i++) {
		...
		pthread_create(&threads[i], NULL, calcular, (void*)taskids[i]);// creacion del hilo
	}
	...
	for (int i = 0; i < MAXTHREAD; i++) {
		...
		sumTotal += partialSums[i]; //Se suma lo que conto cada hilo 
	}
